...
public class PlanBillExtraServiceImpl extends AbstractBillExtraService<PlanBill, ConditionQueryPlanBill> implements PlanBillExtraService {
    ...
    protected Page<PlanBill> pageCondition(ConditionQueryPlanBill conditionQuery, Pageable pageable) {
        ...
        StringBuilder sql = new StringBuilder("select p.bill_id, p.create_time, p.update_time, p.version, p.allot_status, p.audit_memo, p.audit_person_code," +
                "p.audit_state, p.basic_enum, p.belong_station_code, p.bill_code, p.bill_property, p.bill_purpose, p.bill_state, p.bill_type, p.in_or_out_state, p.in_ware_house_time, p.operator_code," +
                " p.out_state_enum, p.out_storage_memo, p.out_ware_house_time, p.plan_memo, p.root_code, p.source_code, p.submit_state, p.total_amount, p.total_price, p.total_variety_amount, p.bill_name, p.hq_bill," +
                " p.operation_state, p.specific_bill_type, d.bill_detail_id, d.actual_amount, d.cargo_code, d.raw_material_code, d.package_code, d.progress, d.shipped_amount, d.in_station_code, d.in_storage_code, d.out_station_code, d.out_storage_code, d.supplier_code, d.bill_code").append(" from plan_bill p left join plan_bill_detail d on p.bill_code = d.bill_code where 1=1 ");
